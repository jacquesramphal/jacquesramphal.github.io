...
description: Internal step of /audit — creates local run folder + screenshots subfolder
...
The filename stays as `step2-setup-drive.md` for compatibility with the skill registry. "Drive" is a misnomer in the current local-only mode — kept to avoid breaking skill discovery. When the Drive-backed backend returns (see `scripts/audit/apps-script/`), this file is swapped back, not renamed.
...
const base = process.env.AUDIT_OUTPUT_BASE || path.join(os.homedir(), "Desktop");
inputs.runFolder = path.join(base, `audit-${safeHost}-${timestamp}`);
```

Example: `/Users/jacques/Desktop/audit-client.com-2026-04-15T15-30-22`

If the user specifically passed a path in Step 1, use that instead and append `audit-<host>-<ts>` as a sub-directory (so running twice into the same target doesn't clobber the previous run).
...
- `scripts/audit/apps-script/Code.gs` — future backend (deferred)
