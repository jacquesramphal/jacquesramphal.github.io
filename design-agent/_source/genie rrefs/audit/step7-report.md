...
- `inputs` — from Step 1 (`mode`, `baseUrl`, `compareAccelerator`, `runId`)
...
```bash
cat > /tmp/genie-audit-${inputs.runId}-audit.json <<EOF
...
with open("/tmp/genie-audit-${inputs.runId}-audit.json") as f:
    rows = json.load(f)
...
```bash
cat > /tmp/genie-audit-${inputs.runId}-consolidation.json <<EOF
...
with open("/tmp/genie-audit-${inputs.runId}-consolidation.json") as f:
    rows = json.load(f)
...
**Accelerator compare:** ${inputs.compareAccelerator ? "Yes" : "No (generic audit)"}

## Components

Total detected: ${rowBuffer.audit.length}

${inputs.compareAccelerator ? `- OOTB matches: ${ootbCount}
...
```bash
rm -f /tmp/genie-audit-${inputs.runId}-audit.json \
      /tmp/genie-audit-${inputs.runId}-consolidation.json
```
...
${inputs.compareAccelerator ? `  • OOTB:              ${ootbCount}
  • With modifications: ${withModsCount}
...
## Phase 2b — planner-agent handoff (if invoked from /prepare)

If `ticketFolder` is in context, also copy `audit-summary.md` into:

```
tickets/<ticketFolder>/genie/component-audit.md
```
...
- **Python 3 missing:** vanishingly rare on modern macOS. If it happens, error:
  ```
  /genie audit needs python3 to write the report. It's pre-installed on macOS 12+.
  If you're on an older system, install Python 3 from python.org.
  ```
- **CSV write fails (permissions, full disk):** preserve the temp JSON at `/tmp/genie-audit-<runId>-<type>.json` and surface the path.
...
- Column schemas: `claude/skills/genie/audit/guides/config.md`
- Future migration (Apps Script path produces `.xlsx` directly via Sheets API): `scripts/genie-audit/apps-script/` (deferred)
