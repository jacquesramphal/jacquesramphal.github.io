...
what was requested, what `compareAccelerator` was set to, or which URLs were in scope.
...
|-----------|-------|
| Mode | `batch` |
| compareAccelerator | `true` or `false` |
| Inventory Sheet ID | `{sheet_id}` (if compareAccelerator=true) |
...
## Phase D — Inventory Matching (if compareAccelerator=true)
...
```js
// 1. Read PROGRESS.md — get original config (URLs, compareAccelerator, run folder)
const progress = Read(`${runFolder}/PROGRESS.md`);
const compareAccelerator = /* read from Run Configuration table */;
...
// 5. If compareAccelerator=true and match files are missing: re-run inventory match
if (compareAccelerator && matchFiles.length < 2) {
...
The first thing written to PROGRESS.md must be the original `/genie audit` invocation
...
**4. compareAccelerator is stateful.**
If the user requests inventory comparison (`compareAccelerator=true`), record it in PROGRESS.md
