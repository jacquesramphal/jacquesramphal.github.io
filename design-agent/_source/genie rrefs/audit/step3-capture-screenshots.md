...
Examples:
- `https://orium.com` → `orium.com-desktop.png`
- `https://orium.com/products/abc-123` → `orium.com_products_abc-123-desktop.png`
...
2. **CSS injection** (`<style id="genie-audit-hide-menus">`) — belt-and-suspenders for elements the inline sweep misses; also covers `:hover` pseudo-class panels
...
const DISMISS_SCRIPT = `async () => {
  ...
  let ex = document.getElementById('genie-audit-hide-menus'); if (ex) ex.remove();
  const s = document.createElement('style'); s.id = 'genie-audit-hide-menus';
  s.textContent = \`
...
const INIT_SCRIPT = `(() => {
  ...
  const STYLE_ID = 'genie-audit-hide-menus';
  const CSS = \`
...
{
  url,
  desktopUrl: desktopCell,     // RELATIVE path for the sheet row (e.g. "screenshots/orium.com-desktop.png")
...
- Future backend: `scripts/genie-audit/apps-script/Code.gs` → `uploadScreenshot()` (deferred)
