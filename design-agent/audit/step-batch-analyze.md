...
description: Internal step of /audit batch — parallel analysis via agents
...
```
You are analyzing {N} URLs for the /audit batch flow. For each URL:

1. Read the desktop + mobile screenshots from disk (paths provided below).
2. Follow the auditor prompt in audit/guides/auditor-prompt.md —
...
3. If inventoryRows is provided, follow the inventory-match prompt in
   audit/guides/inventory-match-prompt.md —
